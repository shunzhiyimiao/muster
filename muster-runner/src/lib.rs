...
//!      `write_file`/`replace_in_file`/`run_command`。写与执行都发生在隔离
//!      分支的独立目录里,主仓一个字节都不动,产出 diff 供人复核。
//!      `run_command`(见 [`command`])与写工具同闸不是巧合:隔离正是它的
//!      安全前提,没有隔离就不该有它——改完代码要能跑一下才知道能不能用,
//!      否则交出去的是一份没人跑过的 diff。
//!
