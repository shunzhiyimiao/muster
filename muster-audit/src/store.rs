...
//! 东西"之间二选一。
//!
//! 正文侧的保留期、按频道/按 run 删除与 JSONL 导出已在桌面壳落地
//! (`transcript.purge` / `transcript.export` 两类事件即其留痕——**删正文
//! 这件事本身也进链**,否则后来的人翻到一段读不到对话的历史,分不清是
//! 被删了还是从来没有过)。
//!
//! 仍未做的是**加密擦除**(正文加密、按需销毁密钥)。当前删除靠
//! DELETE + VACUUM,对付常规取回够用,但抵不过已经拿到旧磁盘镜像的人。
