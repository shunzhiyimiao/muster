...
**决定改用 SSE**(当前代码里还是 WebSocket,待替换)。
...
### Meeting Agent 的技术选择(**未定**)

| 方案 | 优点 | 代价 |
|---|---|---|
| Python sidecar(LiveKit Agents 框架) | 音频帧、VAD、打断处理全现成,快得多 | 项目从纯 Rust 变双语言栈 |
| Rust(livekit Rust SDK) | 栈统一 | 音频那层要自己写 |

治理上两者没差别——不管哪种,转写都得回调 collab-server、走 `muster_route`。
差别纯粹在开发速度与技术栈一致性。
